...
> 🔗 **Built on [KRAFTON KIRA-Slack](https://github.com/krafton-ai/KIRA)** (Apache-2.0). 에이전트 런타임 하니스·Slack 연동·세션 큐잉·오케스트레이터·메모리/프로액티브 서브시스템은 KIRA-Slack에서 파생되었으며, 이 저장소는 그 위에 **실시간 음성 상담(AICC)·Agent Factory 자가확장·OAuth 기반 MCP 서버·서브에이전트 위임·도메인 통합**을 확장한 것입니다. 자세한 출처·변경 내역은 [`NOTICE`](NOTICE), 라이선스는 [`LICENSE`](LICENSE)를 참조하세요.
